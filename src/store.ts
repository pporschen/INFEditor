...
function docReducer(doc: Doc, a: Action): Doc {
	switch (a.type) {
		...
		case "SET_LINE_ARROW":
			return {
				...doc,
				lines: doc.lines.map((l) => (l.id === a.id ? { ...l, arrow: a.arrow === "none" ? undefined : a.arrow } : l)),
			};
		case "SET_LINE_LABEL":
			return {
				...doc,
		...
		case "TOGGLE_ACCEPTING":
			return {
				...doc,
				nodes: doc.nodes.map((n) => (n.id === a.id ? { ...n, accepting: !n.accepting } : n)),
			};
		case "TOGGLE_START":
			return {
				...doc,
		...
		case "SET_TEXT_SIZE":
			return {
				...
				texts: doc.texts.map((t) =>
					t.id === a.id ? { ...t, size: Math.max(0.6, Math.min(4, (t.size ?? 1) + a.delta)) } : t,
				),
			};
		case "SET_TEXT_ALIGN":
			return {
			...
			return {
				...
				tables: doc.tables.map((t) =>
					t.id === a.id
						? {
								...t,
								cells: t.cells.map((row, r) => (r === a.row ? row.map((c, ci) => (ci === a.col ? a.text : c)) : row)),
							}
						: t,
				),
			};
			...
			return {
				...
				tables: doc.tables.map((t) => {
					...
					const cells = t.cells.map((row, r) => {
						...
						const copy = row.slice();
						if (a.delta > 0)
							copy.splice(1, 0, "0"); // new bit column (prefilled 0)
			...
			return {
				...
				tables: doc.tables.map((t) => {
					...
					const toggle = (arr?: number[]) => {
						const cur = arr ?? [];
						return cur.includes(a.index) ? cur.filter((n) => n !== a.index) : [...cur, a.index];
					};
					return a.axis === "col" ? { ...t, boldCols: toggle(t.boldCols) } : { ...t, boldRows: toggle(t.boldRows) };
				}),
			};
		case "TOGGLE_HIGHLIGHT":
			return {
				...
				tables: doc.tables.map((t) => {
					...
					return a.axis === "col" ? { ...t, hlCols: toggle(t.hlCols) } : { ...t, hlRows: toggle(t.hlRows) };
				}),
			};
		...
		case "TOGGLE_STRIKE":
			return {
				...
				tables: doc.tables.map((t) => {
					...
					return {
						...t,
						struck: struck.includes(key) ? struck.filter((k) => k !== key) : [...struck, key],
					};
				}),
			};
		case "TABLE_WIDTH":
			return {
				...doc,
				tables: doc.tables.map((t) => (t.id === a.id ? { ...t, cw: Math.max(1, t.cw + a.delta) } : t)),
			};
		case "TOGGLE_TABLE_HEADER":
			return {
				...doc,
				tables: doc.tables.map((t) => (t.id === a.id ? { ...t, header: !t.header } : t)),
			};
		...
		case "ADD_TABLE_LOOP":
			return {
				...doc,
				tables: doc.tables.map((t) => (t.id === a.id ? { ...t, loops: [...(t.loops ?? []), a.loop] } : t)),
			};
		case "SET_LOOP_LABEL":
			return {
				...doc,
				tables: doc.tables.map((t) =>
					t.id === a.id
						? {
								...t,
								loops: (t.loops ?? []).map((l) => (l.id === a.loopId ? { ...l, label: a.label } : l)),
							}
						: t,
				),
			};
			...
			return {
				...
				tables: doc.tables.map((t) =>
					t.id === a.id
						? {
								...t,
								loops: (t.loops ?? []).map((l) => (l.id === a.loopId ? { ...l, color: a.color } : l)),
							}
						: t,
				),
			};
				...
				tables: doc.tables.map((t) =>
					t.id === a.id
						? {
								...t,
								loops: (t.loops ?? []).map((l) =>
									l.id === a.loopId ? (a.axis === "h" ? { ...l, wrapH: !l.wrapH } : { ...l, wrapV: !l.wrapV }) : l,
								),
							}
						: t,
				),
		...
		case "DEL_TABLE_LOOP":
			return {
				...doc,
				tables: doc.tables.map((t) =>
					t.id === a.id ? { ...t, loops: (t.loops ?? []).filter((l) => l.id !== a.loopId) } : t,
				),
			};
		case "ADD_DERIV":
			...
			return {
				...
				derivations: doc.derivations.map((d) =>
					d.id === a.id
						? {
								...d,
								steps: d.steps.map((s, i) => (i === a.index ? { ...s, [a.field]: a.value } : s)),
							}
						: d,
				),
			};
		...
		case "MOVE_DERIV":
			return {
				...doc,
				derivations: doc.derivations.map((d) => (d.id === a.id ? { ...d, x: a.x, y: a.y } : d)),
			};
		case "MOVE_MANY": {
			...
			const ids = (k: string) => new Set(a.refs.filter((r) => r.kind === k).map((r) => r.id));
			...
			return {
				...doc,
				nodes: doc.nodes.map((n) => (nodeIds.has(n.id) ? { ...n, x: n.x + dx, y: n.y + dy } : n)),
				lines: doc.lines.map((l) =>
					lineIds.has(l.id) ? { ...l, x1: l.x1 + dx, y1: l.y1 + dy, x2: l.x2 + dx, y2: l.y2 + dy } : l,
				),
...
export function useEditor(init: () => Doc) {
	const [hist, dispatch] = useReducer(reducer, undefined, (): History => ({ past: [], present: init() }));
