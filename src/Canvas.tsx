import { forwardRef, useMemo, Fragment, type CSSProperties, type ReactNode } from "react";
import type { Doc, DiagEdge, TableLoop, Mode, Selection, Shape } from "./types";
import { GRID, center, anchor, halfExtents, PAGE_W, PAGE_H, PAGE_MARGIN, pageTop } from "./geometry";
...
export function renderRich(s: string): ReactNode {
	...
	while (i < s.length) {
		...
		if ((ch === "_" || ch === "^") && i + 1 < s.length) {
			...
			const inner = readArg();
			out.push(
				<tspan key={key++} baselineShift={ch === "^" ? "super" : "sub"} fontSize="0.7em">
					{renderRich(inner)}
				</tspan>,
			);
			continue;
...
export const Canvas = forwardRef<SVGSVGElement, Props>(function Canvas(
	...
	const gridLines = (step: number) => {
		const xs: number[] = [];
		const ys: number[] = [];
		for (let x = Math.floor(view.x / step) * step; x <= view.x + view.w; x += step) xs.push(x);
		for (let y = Math.floor(view.y / step) * step; y <= view.y + view.h; y += step) ys.push(y);
		return { xs, ys };
	};
	...
	return (
		...
		>
				...
				<g pointerEvents="none">
					{gridLines(GRID / 4).xs.map((x, i) =>
						Math.round(x / (GRID / 4)) % 4 === 0 ? null : (
							<line key={`mv${i}`} x1={x} y1={view.y} x2={x} y2={view.y + view.h} className="grid-minor" />
						),
					)}
					{gridLines(GRID / 4).ys.map((y, i) =>
						Math.round(y / (GRID / 4)) % 4 === 0 ? null : (
							<line key={`mh${i}`} x1={view.x} y1={y} x2={view.x + view.w} y2={y} className="grid-minor" />
						),
					)}
				</g>
			)}
			...
			<g pointerEvents="none" className="grid">
				{major.xs.map((x, i) => (
					<line key={`v${i}`} x1={x} y1={view.y} x2={x} y2={view.y + view.h} className="grid-line" />
				))}
				{major.ys.map((y, i) => (
					<line key={`h${i}`} x1={view.x} y1={y} x2={view.x + view.w} y2={y} className="grid-line" />
				))}
			</g>
			...
			<g id="content">
				...
				<g {...hitProps}>
					{doc.lines.map((l) => {
						const selected = (selection?.kind === "line" && selection.id === l.id) || multi.has("line:" + l.id);
						const x1 = l.x1 * GRID;
						const y1 = l.y1 * GRID;
						const x2 = l.x2 * GRID;
						const y2 = l.y2 * GRID;
						...
						// regardless of the wire's length. For start/end, also push the
						// label past the endpoint along the tangent so it cannot sit on top
						...
						const tang = pos === "start" ? -along : pos === "end" ? along : 0;
						const off = pos === "middle" ? offMid : offEnd;
						const lx = ax + nx * off + tx * tang;
						const ly = ay + ny * off + ty * tang;
						return (
							<g key={l.id} onClick={() => onLineClick(l.id)} className="edge">
								...
								<line
									...
									markerStart={l.arrow === "start" || l.arrow === "both" ? "url(#arrow)" : undefined}
				...
				<g {...hitProps}>
					{doc.nodes.map((n) => {
						...
						return (
							...
							>
								{n.shape === "dot" ? (
									<circle cx={c.x} cy={c.y} r={hw} className={n.hollow ? "dot-outline" : "dot-fill"} />
								) : n.shape === "circle" ? (
									<>
										<ellipse cx={c.x} cy={c.y} rx={hw} ry={hh} className="node-fill" />
										{n.accepting && (
				...
				<g>
					{doc.tables.map((tb) => {
						const tableSel = (selection?.kind === "table" && selection.id === tb.id) || multi.has("table:" + tb.id);
						const cw = tb.cw * GRID;
						const ch = GRID;
						const px = tb.x * GRID;
						const py = tb.y * GRID;
						...
						if (measureCtx) measureCtx.font = `${fontPx}px system-ui, -apple-system, 'Segoe UI', sans-serif`;
						const textW = (s: string) => (measureCtx ? measureCtx.measureText(s).width : s.length * fontPx * 0.6);
						const colW = Array.from({ length: tb.cols }, (_, c) => {
							if (uniform) return cw;
							let maxW = 0;
							for (let r = 0; r < tb.rows; r++) maxW = Math.max(maxW, textW(tb.cells[r]?.[c] ?? ""));
							...
							<g key={tb.id}>
								...
								{tb.cells.map((row, r) =>
									row.map((cellText, c) => {
										...
										const cx = px + colX[c];
										const cy = py + r * ch;
										const sel = cellSel && cellSel.id === tb.id && cellSel.row === r && cellSel.col === c;
										const loopSel = loopFirst && loopFirst.id === tb.id && loopFirst.row === r && loopFirst.col === c;
										return (
											<g key={`${r}-${c}`} {...hitProps} onClick={() => onCellClick(tb.id, r, c)}>
												<rect
													x={cx}
													y={cy}
													width={colW[c]}
													height={ch}
													className={`table-cell${isHeader ? " table-header" : ""}${sel || loopSel ? " selected" : ""}`}
												/>
												<text
													x={cx + colW[c] / 2}
													y={cy + ch / 2}
                  ...
                  cue driven purely by what the user typed */}
								{tb.checkCol != null &&
									tb.cells.slice(1, -1).map((_, i) => {
										...
										const ly = py + (r + 1) * ch;
										return <line key={`grp-${r}`} x1={px} y1={ly} x2={px + totalW} y2={ly} className="qm-group-sep" />;
									})}
                  ...
                  spans from its first covered minterm to its last */}
								...
								{(tb.boldCols ?? [])
									...
									.map((c) => {
										...
										return (
											<line key={`bc-${c}`} x1={x} y1={py} x2={x} y2={py + tb.rows * ch} className="table-bold-sep" />
										);
									})}
								{(tb.boldRows ?? [])
									.filter((r) => r < tb.rows)
									.map((r) => {
										const y = py + (r + 1) * ch;
										return <line key={`br-${r}`} x1={px} y1={y} x2={px + totalW} y2={y} className="table-bold-sep" />;
									})}
                  ...
                  as mirrored pieces that arc past the opposite edge */}
								{(tb.loops ?? []).map((lp, li, arr) => {
									...
									if (lp.wrapH) pieces.push({ r1: lp.r1, c1: mC(lp.c2), r2: lp.r2, c2: mC(lp.c1) });
									if (lp.wrapV) pieces.push({ r1: mR(lp.r2), c1: lp.c1, r2: mR(lp.r1), c2: lp.c2 });
									if (lp.wrapH && lp.wrapV) pieces.push({ r1: mR(lp.r2), c1: mC(lp.c2), r2: mR(lp.r1), c2: mC(lp.c1) });
									return (
				...
				<g>
					{doc.derivations.map((d) => {
						const dsel = (selection?.kind === "deriv" && selection.id === d.id) || multi.has("deriv:" + d.id);
												...
												{renderLines(
													...
													mode === "select"
														? (e, _li, srcOffset, rawLine) => {
																...
																onExprCaret(d.id, i, srcOffset + Math.round(frac * rawLine.length));
															}
														: undefined,
												)}
											</text>
				...
				<g {...hitProps}>
					{doc.texts.map((t) => {
						const selected = (selection?.kind === "text" && selection.id === t.id) || multi.has("text:" + t.id);
						const px = t.x * GRID;
						const py = t.y * GRID;
						if (t.kind === "text") {
							...
							const lineW = (s: string) => (measureCtx ? measureCtx.measureText(s).width : s.length * fontPx * 0.6);
							...
							return (
								<g key={t.id}>
									...
									<text
										...
										textAnchor={anchor}
										className={`text-block${t.text ? "" : " placeholder"}${t.bold ? " bold" : ""}`}
			...
			{pendingCorner && (
				<g className="ui-only" pointerEvents="none">
					...
					<circle cx={pendingCorner.x * GRID} cy={pendingCorner.y * GRID} r={5} className="preview-corner" />
				</g>
			)}
		</svg>
	);
